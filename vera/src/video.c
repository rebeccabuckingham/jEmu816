...
video_init(int window_scale, float screen_x_scale, char *quality, bool fullscreen, float opacity)
{
  ...
  // TODO bring this back when we have an icon
...
refresh_palette() {
  ...
  printf("refresh_palette() - out_mode is %d\n", out_mode);
...
video_get_irq_out()
{
  // TODO pcm stuff will need to come back 
...
// 					cmd_down = true;
// 				}
// 				handle_keyboard(true, event.key.keysym.mod, event.key.keysym.sym, event.key.keysym.scancode);
// 			}
// 			continue;
// 		}
...
// 				cmd_down = false;
// 			}
// 			handle_keyboard(false, event.key.keysym.mod, event.key.keysym.sym, event.key.keysym.scancode);
// 			continue;
// 		}
...
uint8_t video_read(uint8_t reg, bool debugOn) {
	...
	switch (reg & 0x1F) {
		...
		case 0x07: return isr; // | (pcm_is_fifo_almost_empty() ? 8 : 0);   // TODO bring back
    ...
    // TODO bring these back (all commented out)
		case 0x1B: return 0; //audio_render(); return pcm_read_ctrl();
		case 0x1C: return 0; //pcm_read_rate();
		case 0x1D: return 0;

		case 0x1E:
		case 0x1F: return 0; // vera_spi_read(reg & 1);
	}
	return 0;
}
